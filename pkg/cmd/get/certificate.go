...
verifyctl get personalCert --entitlements`))
...
func newPersonalCertCommand(config *config.CLIConfig, streams io.ReadWriter) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:                 cmdutil.TranslateShortDesc(personalCertMessagePrefix, "Get Verify password policies based on an optional filter or a specific policy."),
